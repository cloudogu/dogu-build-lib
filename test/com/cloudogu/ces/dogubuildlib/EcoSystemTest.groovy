...
class EcoSystemTest {
    ...
    void test_EcoSystem_loginBackend() {
        ...
        verify(vagrantMock).ssh("sudo cesapp login --password-file /tmp/${passwordFile} myUser")
    ...
    void test_EcoSystem_pushPreRelease() {
        ...
        verify(vagrantMock).scp(passwordFile, "/tmp/${passwordFile}")
        verify(vagrantMock).ssh("sudo cesapp login --password-file /tmp/${passwordFile} 'harborUser'")
        verify(vagrantMock).ssh("sudo rm -f /tmp/${passwordFile}")
